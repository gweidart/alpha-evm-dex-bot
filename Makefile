...
hello:
	@echo ""
	@echo "${g}${bd}AlphaBot command line interface${n}${st}"
	@echo ""
...
update: hello
	...
	( echo "${r}[!] You are not using version control system (git). We recommend to use git. AlphaBot.py will be updated and backed up.${n}" && \
	while [ -z "$$CONTINUE" ]; do \
		read -r -p "Are you sure you wish to continue? [y/N] " CONTINUE; \
	done ; \
	( test $$CONTINUE = "y" || test $$CONTINUE = "Y" ) && \
	( mv AlphaBot.py AlphaBot.py.backup && wget https://raw.githubusercontent.com/cusdt-LINK/alpha-kcc-dex-bot/master/AlphaBot.py -O AlphaBot.py && wget https://raw.githubusercontent.com/cusdt-LINK/alpha-kcc-dex-bot/master/requirements.txt -O requirements.txt ) || \
...
build: hello
	...
	@test ! -z "$(shell docker images -q alpha_bot 2> /dev/null)"  && \
	echo "${b}[+] Image ${g}${bd}alpha_bot${st} ${b}exists${n}" || ( \
	echo "${b}[.] Building docker image${n}" && docker build -t alpha_bot . && \
	echo "${b}[+] Docker image successfully built${n}")

prune: hello
	@echo "${b}[.] Check if docker image exists${n}"
	@test -z "$(shell docker images -q alpha_bot 2> /dev/null)"  && \
	echo "${b}[+] Image ${bd}alpha_bot${st} ${b}does not exist${n}" || ( \
	docker rmi alpha_bot && echo "${b}[+] Docker image successfully deleted${n}")
...
%_start: 
	...
	@test "$(shell docker container inspect -f '{{.State.Status}}' alpha_bot_$*)" = "running" && \
	echo "${r}[!] Container ${g}${bd}$*${st}${r} is already running. Skipping...${n}" || ( \
	docker run -d --rm --name alpha_bot_$* \
	-v $(HOME)/instances/$*/settings.json:/app/settings.json -v $(HOME)/instances/$*/tokens.json:/app/tokens.json \
	-v $(HOME)/AlphaBot.py:/app/AlphaBot.py -v $(HOME)/instances/$*/logs/:/app/logs/ \
	--entrypoint '/bin/bash' alpha_bot -c 'python AlphaBot.py ${args}')
...
%_stop: 
	...
	@test "$(shell docker container inspect -f '{{.State.Status}}' alpha_bot_$* )" = "running" && docker stop alpha_bot_$* || ( echo "[!] Container $* is not running. Skipping..." )
...
logs: hello
	...
	@docker logs -f alpha_bot_$(INSTANCE)

ssh: hello
	@echo "${b}[.] Entering shell to the container${n}"
	@docker exec -it alpha_bot_$(INSTANCE) bash